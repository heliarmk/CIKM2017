...
def conv2d(l_input, ksize, bsize, w_decay, padding='SAME'):
...
def conv_lstm(input, step_size, dropout):
    ...
    try:
        ...
        #                               dtype=tf.float32)
        outputs = static_rnn(multi_cells, x)
    # Linear activation, using rnn inner loop last output

    with tf.variable_scope("out_conv1"):
        conv_1 = conv2d(outputs[-1], ksize=[3, 3, filter_n, 128], bsize=[128], w_decay=0.0005)
        relu_1 = tf.nn.relu(conv_1)
        avg_pool_1 = avg_pool(relu_1, k=1)

    with tf.variable_scope("out_conv2"):
        conv_2 = conv2d(avg_pool_1, ksize=[3, 3, 128, 256], bsize=[256], w_decay=0.0005)
        relu_2 = tf.nn.relu(conv_2)
        avg_pool_2 = avg_pool(relu_2, k=2)

    with tf.variable_scope("out_conv3"):
        conv_3 = conv2d(avg_pool_2, ksize=[3, 3, 256, 256], bsize=[256], w_decay=0.0005)
        relu_3 = tf.nn.relu(conv_3)
        avg_pool_3 = avg_pool(relu_3, k=2)

    with tf.variable_scope("out_conv4"):
        conv_4 = conv2d(avg_pool_3, ksize=[3, 3, 256, 256], bsize=[256], w_decay=0.0005)
        relu_4 = tf.nn.relu(conv_4)
        avg_pool_4 = avg_pool(relu_4, k=2)

    with tf.variable_scope("out_conv5"):
        conv_5 = conv2d(avg_pool_4, ksize=[3, 3, 256, 256], bsize=[256], w_decay=0.0005)
    ...
    with tf.variable_scope("out_dense2"):
        fc2 = fully_connected(dropout_fc1, wsize=[1024, 1024], bsize=[1024], w_decay=0.0005)

...
class ConvLSTMCell(tf.contrib.rnn.RNNCell):
    ...
    def __init__(self, shape, filters, kernel, initializer=tf.contrib.layers.xavier_initializer(), forget_bias=1.0,
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or self.__class__.__name__):
            ...
            inputs = tf.concat([inputs, previous_output], axis=self._feature_axis)
            n = channels + filters
            m = gates
...
class ConvGRUCell(tf.contrib.rnn.RNNCell):
    ...
    def __init__(self, shape, filters, kernel, initializer=tf.contrib.layers.xavier_initializer(), activation=tf.tanh,
                 normalize=True):
        self._filters = filters
        self._kernel = kernel
        self._initializer = initializer
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or self.__class__.__name__):
            ...
            with tf.variable_scope('Gates'):
                channels = inputs.shape[-1].value
                inputs = tf.concat([inputs, state], axis=self._feature_axis)
            ...
            with tf.variable_scope('Output'):
                inputs = tf.concat([inputs, reset_gate * state], axis=self._feature_axis)
                n = channels + self._filters
                m = self._filters
                W = tf.get_variable('kernel', self._kernel + [n, m], initializer=self._initializer)
                y = tf.nn.convolution(inputs, W, 'SAME')

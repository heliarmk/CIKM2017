...
def read_data_from_pkl(datafile):
    ...
    inputs = np.array(inputs).reshape(-1, 15, 101, 101, 4).astype(np.float32)
...
def preprocessing(inputs):
    ...
    datas = inputs
    # shape (15,101,101,4)
    mean, var = tf.nn.moments(datas, axes=(1, 2), keep_dims=True)
    datas = tf.subtract(datas, mean)

    # Normalization
    datas = tf.div(datas, tf.sqrt(var))
...
def train():
    ...
    if network_type == "FC":
        ...
        with tf.variable_scope("model") as scope:
            preds, fc7_weights, reg = c3d_fcn(datas, keepprob, train_batch_size, n_class, DTYPE, is_training)
    else:
        raise NotImplementedError

    # some operation
    # loss
    loss, cme, rmse = combind_loss(out_sm, class_labels, out_reg, reg_labels)
